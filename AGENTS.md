...
- **`CameraManager.AvailabilityCallback` fires immediately on registration**: Registering an availability callback triggers `onCameraAvailable()` for each known camera **synchronously** during `registerAvailabilityCallback()`. This can overwrite the AppOp privacy-toggle state emitted by the initial `emitState()` call. **Fix**: Always pass `shouldCheckAppOp = true` (or equivalent) to callback-based `emitState()` calls — don't skip the AppOp check just because the callback isn't about privacy toggles. The same principle applies to `AudioManager.AudioRecordingCallback`.
