...
- Camera/mic use foreground polling (no system callbacks): `observeMicStatus()` and `observeCameraStatus()` take a single snapshot on flow creation, then poll every 2 seconds while collected via `while(true) { delay(2_000L); emitState() }`. When backgrounded, `WhileSubscribed(5_000)` stops collection and polling halts. No `AvailabilityCallback` or `AudioRecordingCallback` is registered — they were removed to avoid spontaneous state changes.
