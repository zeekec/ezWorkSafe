...
- **Semgrep code scanning** (`.github/workflows/semgrep.yml`) — source-based SAST (tree-sitter parser, so no Kotlin-version lag). Runs `p/kotlin` + `p/java` + `p/owasp-top-ten` packs (~560 rules) in the pinned `semgrep/semgrep:` container; `p/android` does NOT exist (HTTP 404) — do not add it. Token-free: `semgrep scan` without `SEMGREP_APP_TOKEN`, `--metrics=off`, no `--error` (findings land in the Security → Code scanning tab as category `semgrep` alongside CodeQL without failing CI). Mandatory `if: github.actor != 'dependabot[bot]'`. Dependabot does not track the container image tag — bump the pin manually. Scan produces ~22 benign/known findings: 19 `github-actions-mutable-action-tag` on workflow action `@vX` tags (kept mutable by design so Dependabot tracks them), 2 `dependabot-missing-cooldown`, and 1 `java.android.security.exported_activity.exported_activity` on the `MainActivity` launcher export (`AndroidManifest.xml:34`). All were **dismissed via the code-scanning API** (09-2026) with per-type reasons (`false positive` for the launcher export; `won't fix` for the rest) — dismissal does NOT suppress the rules, so new drift still surfaces as alerts. Re-dismiss/re-examine after any workflow or manifest change.
- **Dependency Review** (`.github/workflows/dependency-review.yml`) — `actions/dependency-review-action@v4`, gated on `main` PRs at `fail-on-severity: moderate` + `license-check: true` + `comment-summary-in-pr: always`. Consumes the dependency graph fed by the Automatic Dependency Submission (Gradle) check, so it blocks merges that *introduce* a vulnerable/GHSA'd or denied-license dependency before merge (Dependabot only alerts after code lands). Registered as a **required status check** (context `dependency-review`, the job name) in the ProtectBranches ruleset. Requires `pull-requests: write` for the in-PR summary comment.
- **Release** (`.github/workflows/release.yml`) — tag-driven: pushing `v*` (e.g. `git push origin v0.2.0`) runs lint + unit tests, builds the signed APK, and creates a GitHub Release with `--generate-notes` (needs `contents: write`). The APK's `versionName`/`versionCode` are stamped from the tag (`-PversionName`/`-PversionCode`, computed as `major*10000+minor*100+patch`), so releases always match their tag. `VERSION_NAME`/`VERSION_CODE` in `gradle.properties` only affect local builds. Fails fast (exit 1, no release) if `KEYSTORE_B64`/`KEYSTORE_PROPERTIES_B64` secrets are absent, so an unsigned APK is never published. `--generate-notes` bases notes on merged PRs since the previous tag — the first tag after adopting this workflow gives full-history notes.
