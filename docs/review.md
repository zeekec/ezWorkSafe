...
- **Status: ✓ FIXED** — Added `-keep class com.ezworksafe.widget.** { *; }` to `proguard-rules.pro` (PR #?).
...
| Missing Glance ProGuard keep rules | ✓ Added `-keep class com.ezworksafe.widget.**` to `proguard-rules.pro` (PR #?) |
...
- ~~**1 medium:** Missing Glance ProGuard keep rules may break release builds~~ ✓ Fixed (PR #?)
